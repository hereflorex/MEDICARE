...
app.config['SESSION_COOKIE_NAME'] = '__Host-medicare_session' if app.config['SESSION_COOKIE_SECURE'] else 'medicare_session'
...
class UserPresence(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'role', name='uq_presence_user_role'),
    )
...
def current_user():
    ...
    if not role or not user_id or role == 'admin':
        return None
    if role == 'doctor':
...
def mark_presence():
    user = current_user()
    role = session.get('role')
    if not user or role not in {'patient', 'doctor'}:
        return
    row = UserPresence.query.filter_by(user_id=user.id, role=role).first()
    if row:
        row.last_seen = datetime.utcnow()
    else:
        db.session.add(UserPresence(user_id=user.id, role=role, last_seen=datetime.utcnow()))
    try:
        db.session.commit()
    except Exception:
        db.session.rollback()
...
def security_and_presence():
    ...
    if request.method in {'POST', 'PUT', 'PATCH', 'DELETE'}:
        if not valid_csrf():
            ...
            return render_template('admin_login.html', error='Security token expired. Refresh the page and try again.'), 403
    if request.path.startswith('/api/presence'):
        return None
    if request.method == 'GET' and request.path not in {'/admin/login'}:
        if session.get('role') in {'patient', 'doctor'} and session.get('user_id'):
            mark_presence()
    return None
...
def security_headers(response):
    ...
    if request.path.startswith(('/admin', '/dashboard', '/profile', '/appointments', '/prescriptions', '/billing', '/api/')):
...
def admin_login():
    if request.method == 'POST':
        ...
        recent = [t for t in _admin_failures.get(ip, []) if now - t < 600]
        _admin_failures[ip] = recent
        if len(recent) >= 5:
        ...
        if not admin_email or not admin_password:
            return render_template('admin_login.html', error='Admin credentials are not configured on the server.'), 503
...
def admin_panel():
    ...
    active_users = UserPresence.query.filter(UserPresence.last_seen >= now - timedelta(minutes=5)).count()
    registrations = []
    for user in Patient.query.order_by(Patient.created_at.desc()).limit(8).all():
        registrations.append({'name': user.name, 'email': user.email, 'role': 'patient', 'created_at': user.created_at or now})
    for user in Doctor.query.order_by(Doctor.created_at.desc()).limit(8).all():
        registrations.append({'name': user.name, 'email': user.email, 'role': 'doctor', 'created_at': user.created_at or now})
    registrations.sort(key=lambda x: x['created_at'], reverse=True)
    active_presence = UserPresence.query.filter(
        UserPresence.last_seen >= now - timedelta(minutes=5)
    ).order_by(UserPresence.last_seen.desc()).limit(12).all()
    active_people = []
    for presence in active_presence:
        model = Doctor if presence.role == 'doctor' else Patient
        user = db.session.get(model, presence.user_id)
        if user:
    ...
    return render_template('admin.html', stats=stats, recent=recent, registrations=registrations[:10], active_people=active_people)
...
def admin_live():
    ...
    active_rows = UserPresence.query.filter(
        UserPresence.last_seen >= cutoff
    ).order_by(UserPresence.last_seen.desc()).limit(12).all()
    active_people = []
    for row in active_rows:
        model = Doctor if row.role == 'doctor' else Patient
        user = db.session.get(model, row.user_id)
        if user:
            active_people.append({
                'name': user.name,
                'email': user.email,
    ...
    return jsonify({
        'active_users': UserPresence.query.filter(UserPresence.last_seen >= cutoff).count(),
        'patients': Patient.query.count(),
        'doctors': Doctor.query.count(),
        'total_users': Patient.query.count() + Doctor.query.count(),
        'appointments': Appointment.query.count(),
        'pending_bills': Invoice.query.filter_by(status='pending').count(),
...
def login():
    ip = request.headers.get('X-Forwarded-For', request.remote_addr or 'unknown').split(',')[0].strip()
    now = time.time()
    recent = [t for t in _auth_failures.get(ip, []) if now - t < 600]
...
def create_prescription():
    ...
    has_care_relationship = Appointment.query.filter_by(
        doctor_id=current_user().id,
        patient_id=patient_id
    ).first()

    if not has_care_relationship:
        return jsonify({
            'error': 'You can only prescribe for a patient connected to your appointments'
        }), 403
